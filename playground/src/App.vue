...
const initialVersions: Versions = {
  ...
  [playConfig.compLibShort]:
    (IS_DEV ? `@${__COMMIT__}` : params.get(playConfig.compLibShort)) ||
    'latest',
  ...
  (event) => {
    if (typeof event.data === 'string') {
      generate(event.data, (css: string) => {
        ...
        if (frame.length > 0) frame[0].contentWindow?.postMessage(css, '*')
      })
    }
  },
  false,
  ...
  <div class="comp-lib-play">
    ...
    >
      <div flex items-center space-x-3>
        ...
        <p class="text-xl">Loading Luma UI playground</p>
      </div>
    </div>
  </div>
</template>
