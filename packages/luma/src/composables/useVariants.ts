...
function mergeClasses(...classes: CSSClasses): string {
  return classes
    .map((className: CSSClass): string => {
      ...
      if (Array.isArray(className)) return mergeClasses(...className);
...
export function useVariants<T>(
  ...
  const config = inject<LumaUIConfiguration>(
    "config",
    {} as LumaUIConfiguration,
  );
  ...
  if (globalVariant) {
    for (const [key, value] of Object.entries(globalVariant.base || {})) {
      const result = props.base?.[key] ? [props.base[key]] : [value];
