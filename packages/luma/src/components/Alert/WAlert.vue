...
const transition = computed(() => {
  return props.transition === "scale"
    ? variant.value.transitions?.scale
    : props.transition === "fade"
      ? variant.value.transitions?.fade
      : props.transition === "slideLeft"
        ? variant.value.transitions?.slideLeft
        : variant.value.transitions?.slideRight;
});
    ...
    <div v-if="props.isVisible" :class="variant.root">
      <div :class="variant.alertFlexBetween">
        <div
          :class="[
            $slots.leading || isLeading
              ? variant.alertIsLeading
              : variant.alertIsNotLeading,
          ]"
        ...
        >
          <div class="shrink-0">
            <p
              ...
              :class="variant.alertIconIsLeading"
            >
              <slot name="leading">
        ...
        <div>
          <div class="shrink-0">
            <button
              v-if="props.closable && !isTrailing && !$slots.trailing"
              :title="dismissLabel"
              :aria-label="dismissLabel"
              :class="variant.alertCloseButtonClass"
              @click="onDismiss()"
            >
              <XMarkIcon
                class="w-6 h-6 hover:text-white"
                :class="variant.alertCloseIcon"
              />
            </button>

            <button
              v-if="props.trailing || isTrailing || $slots.trailing"
              :title="dismissLabel"
              :aria-label="dismissLabel"
              :class="variant.alertTrailingClass"
              @click="onDismiss()"
            >
