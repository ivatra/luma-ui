...
export type AvatarChipPosition =
  | "top-right"
  | "top-left"
  | "bottom-right"
  | "bottom-left";
...
watchEffect(() => {
  ...
  img
    .decode()
    .then(() => (avatarUrl.value = props.src))
    .catch((err: string) => {
      avatarUrl.value = "";
      throw err;
    });
...
const avatarChipSize = computed(() => {
  ...
  if (typeof props.size === "number") {
    ...
    sizeClass += lumaTheme.WAvatar.base.avatarChipSize[props.size] || "";
...
const avatarChipClass = computed(() => {
  return classNames(
    ...
    lumaTheme.WAvatar.base.avatarChipPosition[props.chipPosition],
  );
});

const avatarIconSize = computed(() => {
  return classNames(lumaTheme.WAvatar.base.avatarIconSize[props.size]);
});
...
<template>
  <span :class="[avatarWrapperClasses, avatarClasses]" :title="props.name">
    <img
      ...
      :alt="props.name"
    />
    <span v-else-if="!avatarUrl" :class="variant.avatarPlaceholderClass">{{
      fallback
    }}</span>
    <Icon
      v-if="!avatarUrl && !fallback"
      :icon="props.icon"
      :class="[avatarIconSize, variant.avatarIconColor]"
    />
    <span
      v-if="props.chipColor"
      :style="avatarChipColorStyles"
      :class="[avatarChipClass, avatarChipSize]"
    >
      {{ chipText }}
    </span>
    <slot />
