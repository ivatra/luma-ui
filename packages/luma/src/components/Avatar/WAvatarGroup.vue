...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const max = computed(() =>
      typeof props.max === "string" ? Number.parseInt(props.max) : props.max,
    );
    ...
    const clones = computed(() =>
      children.value
        .map(
          (
            node: VNode<unknown, unknown, { [key: string]: any }>,
            index: number,
          ) => {
            ...
            if (!props.max || (max.value && index < max.value)) {
              ...
              vProps.class += `${classNames(
                lumaTheme.WAvatarGroup.base.avatarGroupMargin,
              )}`;
    ...
    // @ts-expect-error
    return () =>
      h("div", { class: lumaTheme.WAvatarGroup.base.root }, clones.value);
  },
});
</script>
