...
const badgeValue = computed(() => {
  if (props.chip || !props.value) return;

  if (props.maxValue !== null)
    return Number(props.value) > props.maxValue
      ? `${props.maxValue}+`
      : props.value;
...
const badgePosition = computed(() => {
  return classNames(
    slots.default && lumaTheme.WBadge.base.badgePosition[props.position],
  );
});

const badgeChip = computed(() => {
  return classNames(props.chip && variant.value.badgeChip);
});

const badgeSquare = computed(() => {
  return classNames(props.square && variant.value.badgeSquare);
});
</script>
  ...
  <div :class="variant.root">
    <span :class="variant.default">
      ...
      <Transition v-bind="variant.transitions?.fade">
        <span
          :class="[variant.badgeBody, badgePosition, badgeChip, badgeSquare]"
        >
