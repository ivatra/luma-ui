...
const accordionItem = computed(() => {
  return classNames(
    show.value
      ? [
          variant.value.accordionItemActiveBackgroundColor,
          variant.value.accordionItemActiveTitleColor,
          variant.value.accordionItemButton,
        ]
      : variant.value.accordionItemButton,
  );
});
...
const isDisabled = computed(() => {
  return classNames(props.disabled && variant.value.accordionIsDisabled);
});
</script>
...
<template>
  <div>
    <div :class="[variant.root, props.rounded && variant.accordionItemRounded]">
      <div
        :class="[accordionItem, isDisabled]"
        :aria-expanded="show"
        @click="toggle"
      >
        <div
          :class="[
            $slots.leading || isLeading
              ? variant.accordionItemIconIsLeading
              : variant.accordionItemIconIsNotLeading,
          ]"
          ...
          <div class="shrink-0">
            <span v-if="isLeading || $slots.leading" class="space-x-2">
              <slot name="leading">
                <Icon
                  :icon="leadingIcon"
                  :class="variant.accordionItemActiveLeadingIconColor"
                />
              </slot>
            </span>
          </div>
          <div>
            <div>
              <div :class="variant.accordionItemTitle">
                <h2
                  :class="[
                    show ? variant.accordionItemActiveTitleColor : isDisabled,
                  ]"
                >
                  {{ props.title }}
                </h2>
        ...
        <div
          ...
          :class="variant.accordionItemIcon"
        >
          <Icon
            icon="material-symbols:keyboard-arrow-down"
            :class="[
              show
                ? `${variant.accordionItemActiveTitleColor} rotate-180 ${variant.accordionItemIcon}`
                : `${variant.accordionItemIcon}`,
        ...
        <div v-if="isTrailing || $slots.trailing">
          <slot name="trailing" :active="show">
            <Icon
              :icon="trailingIcon"
              :class="variant.accordionItemActiveTrailingIconColor"
            />
          </slot>
        </div>
      </div>
