...
const transitionClassWrapper = computed(() => {
  ...
  return {
    ...lumaTheme.WModal.base.modalOverlayTransition,
  };
});
...
<template>
  <TransitionRoot :appear="appear" :show="isOpen" as="template">
    <Dialog
      :class="variant.root"
      @close="(e: boolean) => !disableClose && close(e)"
    >
      <TransitionChild
        v-if="overlay"
        as="template"
        :appear="appear"
        v-bind="transitionOverlay"
      >
        <div
          :class="[variant.modalOverlayBase, variant.modalOverlayBackground]"
        />
      </TransitionChild>
      ...
      <div :class="variant.modalInner">
        <div :class="[variant.modalContainer]">
          <TransitionChild
            as="template"
            :appear="appear"
            v-bind="transitionClassWrapper"
          >
            <DialogPanel
              :class="[
                variant.modalPadding,
                variant.modalBase,
                variant.modalHeight,
                variant.modalWidth,
                variant.modalShadow,
